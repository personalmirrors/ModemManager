...
QmiClient *
mm_broadband_modem_qmi_peek_client (MMBroadbandModemQmi  *self,
                                    QmiService            service,
                                    GError              **error)
{
    MMPortQmi *port;
...
gboolean
mm_broadband_modem_qmi_ensure_client (MMBroadbandModemQmi *self,
                                      QmiService service,
                                      QmiClient **o_client,
                                      ...
                                      gpointer user_data)
{
    ...
    client = mm_broadband_modem_qmi_peek_client (self, service, &error);
    if (!client) {
        g_task_report_error (self, callback, user_data, mm_broadband_modem_qmi_ensure_client, error);
        return FALSE;
    }
...
load_unlock_required_context_step (GTask *task)
{
    ...
    case LOAD_UNLOCK_REQUIRED_STEP_DMS:
        if (!self->priv->dms_uim_deprecated) {
            ...
            client = mm_broadband_modem_qmi_peek_client (self, QMI_SERVICE_DMS, &error);
            if (!client) {
                g_task_return_error (task, error);
                g_object_unref (task);
                return;
    ...
    case LOAD_UNLOCK_REQUIRED_STEP_UIM:
        ...
        client = mm_broadband_modem_qmi_peek_client (self, QMI_SERVICE_UIM, &error);
        if (!client) {
            g_task_return_error (task, error);
            g_object_unref (task);
            return;
                             ...
                             GTask               *task)
{
    ...
    client = mm_broadband_modem_qmi_peek_client (self, QMI_SERVICE_DMS, NULL);
    if (!client) {
...
{
    MMIfaceModemLocation *impl;
    iface_modem_location_parent = g_type_interface_peek_parent (iface);

    impl = mm_broadband_modem_qmi_location_pds(iface_modem_location_parent);

    iface->load_capabilities = impl->load_capabilities;
    iface->load_capabilities_finish = impl->load_capabilities_finish;
    iface->load_supl_server = impl->load_supl_server;
    iface->load_supl_server_finish = impl->load_supl_server_finish;
    iface->set_supl_server = impl->set_supl_server;
    iface->set_supl_server_finish = impl->set_supl_server_finish;
    iface->enable_location_gathering = impl->enable_location_gathering;
    iface->enable_location_gathering_finish = impl->enable_location_gathering_finish;
    iface->disable_location_gathering = impl->disable_location_gathering;
    iface->disable_location_gathering_finish = impl->disable_location_gathering_finish;
}
